...
import { FileText, Mail, User, AlertCircle } from "lucide-react";
import {
  logoutMentor,
  mentorGetAllMentees,
  mentorGetDetails,
...
import MentorProofDashboard from "./dashboardLinks/MentorProofDashboard";
...
const MentorDashboard = () => {
  ...
  useEffect(() => {
    ...
    const handleNewNoti = (data) => {
      ...
      if (
        ...
        dispatch(
          markNotificationRead(history, [{ id: data._id, willReceive: false }])
        );
      } else {
  ...
  useEffect(() => {
    ...
    const handleMsgRec = (data) => {
      ...
      if (
        localStorage.getItem("selectedChat") === data.data.chat._id.toString()
      ) {
        if (
        ...
        if (!JSON.parse(localStorage.getItem("chatRoute")))
          setNewMsgNotify(true);
  ...
  const handleRouteChange = (e) => {
    const selectedTab = e.target.id || e;
    // Reset all routes to false first
    const resetRoutes = {
      home: false,
      post: false,
      menteeInfo: false,
  ...
  const handleMentorProofClick = () => {
    ...
    // Set only mentorProof route to true
    setRoute({
      ...resetRoutes,
      mentorProof: true,
    });
        ...
        <authContext.Provider value={{ uid, role }}>
          <div className="h-screen flex bg-gray-50 overflow-hidden">
            <nav className="w-3/20 h-screen bg-white flex flex-col z-10">
              ...
              {role === Roles.ADMIN && (
                <button
                  onClick={handleMentorProofClick}
                  id="mentor-proof"
                  className={`${
                    route.mentorProof
                      ? "text-gray-700 bg-gray-100"
                ...
                >
                  <FileText
                    className={`h-5 w-5 mr-3 pointer-events-none ${
                      route.mentorProof ? "text-blue-600" : ""
                    }`}
                  />
                  Mentor Proofs
                </button>
            ...
            <div className="w-17/20 h-screen">
                        ...
                        <img
                          src={
                            adminData?.avatar?.url === ""
                              ? `https://api.dicebear.com/9.x/personas/svg?seed=${adminData?.email}`
                              : adminData?.avatar?.url
                          }
                          alt="avatar"
                        ...
                        <img
                          src={
                            profileData?.avatar?.url === ""
                              ? `https://api.dicebear.com/9.x/personas/svg?seed=${profileData?.email}`
                              : profileData?.avatar?.url
                          }
                          alt="avatar"
                          className="w-14 h-14 rounded-full"
              ...
              <div
                className={`h-9/10 bg-gradient-to-br from-blue-50 to-indigo-50 overflow-hidden}`}
              >
