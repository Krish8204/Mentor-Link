...
module.exports = {
    ...
    getmentorproof: async (req, res) => {
        try {
            // Retrieve all mentors with selected fields
            const mentors = await Mentor.find({}, {
                firstName: 1,
                lastName: 1,
                email: 1,
                'qualificationProof.url': 1
            });
            ...
            if (!mentors || mentors.length === 0) {
                return response.success(res, "No mentors found", []);
            }
    
            // Transform mentors data for response
            const mentorProofs = mentors.map(mentor => ({
                id: mentor._id,
                firstName: mentor.firstName,
                lastName: mentor.lastName,
                email: mentor.email,
                qualificationProofUrl: mentor.qualificationProof?.url || null
            }));
    
            // Send successful response
            response.success(res, "Mentor proofs retrieved successfully", mentorProofs);
    
        } catch (error) {
            console.error('Error in getmentorproof:', error);
            response.error(res, "Failed to retrieve mentor proofs", error);
        }
    },
